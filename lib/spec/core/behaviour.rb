  ...
  module Core
    class Behaviour
      ...
      def self.inherited(klass)
        ...
        Spec::Core.world.behaviours << klass
      end
      ...
      def self.example(desc=nil, options={}, &block)
        examples << Spec::Core::Example.new(self, desc, options.update(:caller => caller[0]), block)
      end
      ...
      def self.alias_example_to(new_alias, extra_options={})
        new_alias = <<-END_RUBY
                      def self.#{new_alias}(desc=nil, options={}, &block)
                        ...
                        examples << Spec::Core::Example.new(self, desc, updated_options, block)
                      end
                    END_RUBY
      ...
      def self.set_it_up(*args)
        ...
        Spec::Core.configuration.find_modules(self).each do |include_or_extend, mod, opts|                                                                                                                                                                               
          if include_or_extend == :extend
            Spec::Core.configuration.trace { "Extending module #{mod} on #{self}" }
            send(:extend, mod) unless extended_modules.include?(mod)                                                                                                                                                                                                    
          else
            Spec::Core.configuration.trace { "Including module #{mod} on #{self}" }
      ...
      def self.ancestors(superclass_last=false)
        ...
        while current_class < Spec::Core::Behaviour
      ...
      def self.eval_before_alls(running_behaviour)
        ...
        Spec::Core.configuration.find_before_or_after(:before, :all, self).each { |blk| running_behaviour.instance_eval(&blk) }
      ...
      def self.eval_before_eachs(running_behaviour)
        Spec::Core.configuration.find_before_or_after(:before, :each, self).each { |blk| running_behaviour.instance_eval(&blk) }
      ...
      def self.eval_after_alls(running_behaviour)
        ...
        Spec::Core.configuration.find_before_or_after(:after, :all, self).each { |blk| running_behaviour.instance_eval(&blk) }
      ...
      def self.eval_after_eachs(running_behaviour)
        ...
        Spec::Core.configuration.find_before_or_after(:after, :each, self).each { |blk| running_behaviour.instance_eval(&blk) }
      end
      ...
      def self.to_s
        self == Spec::Core::Behaviour ? 'Spec::Core::Behaviour' : name
      end
