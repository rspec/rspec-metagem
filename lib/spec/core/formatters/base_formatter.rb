  ...
  module Core
    ...
    module Formatters
      ...
      class BaseFormatter
        ...
        def output
          Spec::Core.configuration.output
        end

        def trace(&blk)
          Spec::Core.configuration.trace(trace_override_flag, &blk)
        end
        ...
        def profile_examples?
          Spec::Core.configuration.profile_examples
        end
        ...
        def pending_examples
          @pending_examples ||= ::Spec::Core.world.find(examples, :positive, :execution_result => { :status => 'pending' })
        end

        def failed_examples
          @failed_examples ||= ::Spec::Core.world.find(examples, :positive, :execution_result => { :status => 'failed' })
        end
