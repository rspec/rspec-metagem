...
describe Spec::Core::Behaviour do
  ...
  describe "describing behaviour with #describe" do
    ...
    example "an ArgumentError is raised if no type or description is given" do
      lambda { Spec::Core::Behaviour.describe() {} }.should raise_error(ArgumentError, "No arguments given.  You must a least supply a type or description")
    end

    example "an ArgumentError is raised if no block is given" do
      lambda { Spec::Core::Behaviour.describe('foo') }.should raise_error(ArgumentError, "You must supply a block when calling describe")
    end
    ...
    describe '#name' do
      ...
      it "should expose the first parameter as name" do
        isolate_behaviour do
          Spec::Core::Behaviour.describe("my favorite pony") { }.name.should == 'my favorite pony'
        end
      end

      it "should call to_s on the first parameter in case it is a constant" do
        isolate_behaviour do
          Spec::Core::Behaviour.describe(Object) { }.name.should == 'Object'
        end
      end
      ...
      pending "should build a nested name correctly, including the module, and each nested description in the correct order" do
        ...
        isolate_behaviour do
          Spec::Core::Behaviour.describe(Spec::Core, "test") do
            Spec::Core::Behaviour.describe("nested one") do
              behaviour_to_test = Spec::Core::Behaviour.describe("nested two") { }
            end
          end
        end
        
        behaviour_to_test.name.should == 'Spec::Core - test - nested one - nested two'
      end
    ...
    describe '#describes' do
      ...
      it "should be the first parameter when it is a constant" do
        isolate_behaviour do
          Spec::Core::Behaviour.describe(Object) { }.describes.should == Object
        end
      end

      it "should be nil when the first parameter is a string" do
        isolate_behaviour do
          Spec::Core::Behaviour.describe("i'm a computer") { }.describes.should be_nil
        end
      end
    ...
    describe '#description' do
      ...
      it "should expose the second parameter as description" do
        isolate_behaviour do
          Spec::Core::Behaviour.describe(Object, "my desc") { }.description.should == 'my desc'
        end
      end

      it "should allow the second parameter to be nil" do
        isolate_behaviour do
          Spec::Core::Behaviour.describe(Object, nil) { }.description.size.should == 0
        end
      end
    ...
    describe '#metadata' do
      ...
      it "should add the third parameter to the metadata" do
        isolate_behaviour do
          Spec::Core::Behaviour.describe(Object, nil, 'foo' => 'bar') { }.metadata.should include({ "foo" => 'bar' })
        end
      end

      it "should add the caller to metadata" do
        isolate_behaviour do
          Spec::Core::Behaviour.describe(Object) { }.metadata[:behaviour][:caller][4].should =~ /#{__FILE__}:#{__LINE__}/
        end
      end

      it "should add the the file_path to metadata" do
        isolate_behaviour do
          Spec::Core::Behaviour.describe(Object) { }.metadata[:behaviour][:file_path].should == __FILE__
        end
      end

      it "should have a reader for file_path" do
        isolate_behaviour do
          Spec::Core::Behaviour.describe(Object) { }.file_path.should == __FILE__
        end
      end

      it "should add the line_number to metadata" do
        isolate_behaviour do
          Spec::Core::Behaviour.describe(Object) { }.metadata[:behaviour][:line_number].should == __LINE__
        end
      end

      it "should add file path and line number metadata for arbitrarily nested describes" do
        Spec::Core::Behaviour.describe(Object) do
          Spec::Core::Behaviour.describe("foo") do
            Spec::Core::Behaviour.describe(Object) { }.metadata[:behaviour][:file_path].should == __FILE__
            Spec::Core::Behaviour.describe(Object) { }.metadata[:behaviour][:line_number].should == __LINE__
          end
        end
  ...
  describe "#run_examples" do
    ...
    before do
      @fake_formatter = Spec::Core::Formatters::BaseFormatter.new
    end
    ...
    it "should return true if all examples pass" do
      use_formatter(@fake_formatter) do
        passing_example1 = Spec::Core::Example.new(stub_behaviour, 'description', {}, (lambda { 1.should == 1 }))
        passing_example2 = Spec::Core::Example.new(stub_behaviour, 'description', {}, (lambda { 1.should == 1 }))
        Spec::Core::Behaviour.stubs(:examples_to_run).returns([passing_example1, passing_example2])

        Spec::Core::Behaviour.run_examples(stub_behaviour, stub_everything('reporter')).should be_true
      end
    end

    it "should return false if any of the examples return false" do
      use_formatter(@fake_formatter) do
        failing_example = Spec::Core::Example.new(stub_behaviour, 'description', {}, (lambda { 1.should == 2 }))
        passing_example = Spec::Core::Example.new(stub_behaviour, 'description', {}, (lambda { 1.should == 1 }))
        Spec::Core::Behaviour.stubs(:examples_to_run).returns([failing_example, passing_example])

        Spec::Core::Behaviour.run_examples(stub_behaviour, stub_everything('reporter')).should be_false
      end
    end
    ...
    it "should run all examples, regardless of any of them failing" do
      use_formatter(@fake_formatter) do
        ...
        Spec::Core::Behaviour.run_examples(stub_behaviour, stub_everything('reporter'))
      end
    end
