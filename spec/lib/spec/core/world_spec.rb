...
describe Spec::Core::World do
  
  before do
    @world = Spec::Core::World.new
    Spec::Core.stubs(:world).returns(@world)
  end

  describe "behaviour groups" do
  
    it "should contain all defined behaviour groups" do
      behaviour_group = Spec::Core::Behaviour.describe(Bar, 'Empty Behaviour Group') { }
  ...
  describe "applying inclusion filters" do
    ...
    before(:all) do
      ...
      @bg1 = Spec::Core::Behaviour.describe(Bar, "find group-1", options_1) { }
      @bg2 = Spec::Core::Behaviour.describe(Bar, "find group-2", options_2) { }
      @bg3 = Spec::Core::Behaviour.describe(Bar, "find group-3", options_3) { }
      @bg4 = Spec::Core::Behaviour.describe(Foo, "find these examples") do
    ...
    after(:all) do
      Spec::Core.world.behaviours.delete(@bg1)
      Spec::Core.world.behaviours.delete(@bg2)
      Spec::Core.world.behaviours.delete(@bg3)
      Spec::Core.world.behaviours.delete(@bg4)
    end
  ...
  describe "applying exclusion filters" do
    ...
    it "should find nothing if all describes match the exclusion filter" do
      ...
      isolate_behaviour do
        group1 = Spec::Core::Behaviour.describe(Bar, "find group-1", options) do
          it("foo") {}
          it("bar") {}
        end
      ...
      isolate_behaviour do
        group2 = Spec::Core::Behaviour.describe(Bar, "find group-1") do
    ...
    it "should find nothing if a regexp matches the exclusion filter" do
      isolate_behaviour do
        group = Spec::Core::Behaviour.describe(Bar, "find group-1", :name => "exclude me with a regex", :another => "foo") do
          it("foo") {}
          it("bar") {}
        end
  ...
  describe "filtering behaviours" do
    ...
    before(:all) do
      @group1 = Spec::Core::Behaviour.describe(Bar, "find these examples") do
    ...
    after(:all) do
      Spec::Core.world.behaviours.delete(@group1)
    end

    it "should run matches" do
      Spec::Core.world.stubs(:exclusion_filter).returns({ :awesome => false })
      Spec::Core.world.stubs(:filter).returns({ :color => :red })
      Spec::Core.world.stubs(:behaviours).returns([@group1])
