Feature: Define a custom matcher
  ...
  Behind the scenes `RSpec::Matchers.define` creates a Singleton-class for you. If you need to write a more complex matcher and would like to use the `Class`-approach yourself, please head over to our `API`-documentation and read the docs about our `MatcherProtocol`: http://rspec.info/documentation/latest/rspec-expectations/RSpec/Matchers/MatcherProtocol.html.
